...
- **이미 코드가 있다 → 빠른 경로 우선**: `html.to.design` 플러그인으로 돌아가는 `localhost:5173`을 편집 가능한 레이어로 임포트 → Variables/컴포넌트로 정리. AI 텍스트→디자인(First Draft / Figma Make)은 **변형 탐색용**으로만.
...
- 탭 전환은 좌우 슬라이드(내 기록◀홈▶기타 순서), 등장은 ease-out 160~400ms, `reduced-motion` 고려.
...
**Splash**
```
앱 진입 스플래시. 화면 중앙에 회전하는 3D 캡슐 알약(좌 민트 / 우 화이트, 광택), 그 아래 큰
타이틀 "약속", 부제 "잊지 않게, 안심하게". 은은한 민트 방사형 그라데이션 배경. 미니멀·차분.
```

**Home (카메라)**
```
알약 촬영 홈. 중앙에 카메라 뷰파인더 카드(흰색, 라운드 24, 약한 그림자) — 네 모서리 민트 코너
브래킷 + 옅은 알약 실루엣 가이드. 캡션 "가운데 칸에 알약을 맞춰 주세요". 주 버튼(민트) "알약
촬영하기". 아래에 촬영 팁 카드(체크 불릿 3개). 하단 탭바(내 기록·홈·기타). 상태: 대기/인식중/권한거부.
```
...
- [ ] (빠른 길) `html.to.design`로 `localhost:5173` 8화면 임포트 → 레이어 정리
- [ ] 반복 요소를 **Component**로(Button·Card·기록카드·링카드·warn박스·TabBar·Input·Chip)
- [ ] `Screens`에서 컴포넌트 조합으로 8화면 구성(440×932)
